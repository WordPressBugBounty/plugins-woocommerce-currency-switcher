...
final class WOOCS {
	...
	public function __construct() {
		...
		if ( ! intval( get_option( 'woocs_first_activation', 0 ) ) ) {
			...
			update_option( 'woocs_separators', array() );
			...
			$geoIP_profiles = array(
				'woocs_profile_def_p777' => array(
					'name' => esc_html__( 'EU countries', 'woocommerce-currency-switcher' ),
					'data' => array( 'AT', 'BE', 'BG', 'CY', 'CZ', 'DE', 'DK', 'EE', 'ES', 'FI', 'FR', 'GR', 'HR', 'HU', 'IE', 'IT', 'LT', 'LU', 'LV', 'MT', 'NL', 'PO', 'PT', 'RO', 'SE', 'SI', 'SK' ),
				),
			);
	...
	public function init() {
		...
		try {
			...
			if ( function_exists( 'determine_locale' ) ) {
				...
				} elseif ( is_file( WOOCS_PATH . "languages/woocommerce-currency-switcher-{$locale}.mo" ) ) {
		...
		if ( is_admin() and ! wp_doing_ajax() ) {
			...
			} elseif ( ! ( isset( $_POST['action'] ) && $_POST['action'] == 'editpost' && isset( $_POST['post_type'] ) && $_POST['post_type'] == 'shop_order' ) ) {
					$this->current_currency = $this->default_currency;
			}
		...
		add_filter(
			...
			function ( $data ) {
				foreach ( $data as $k => $d ) {
					if ( isset( $d['id'] ) ) {
						if ( in_array( $d['id'], array( 'woocommerce_currency', 'woocommerce_price_num_decimals', 'woocommerce_currency_pos' ) ) ) {
							// unset($data[$k]);
						}
	...
	public function woocs_order_metabox( $post ) {
		...
		wp_localize_script(
			...
			'woocs_meta_params',
			array(
				'currency_nonce' => wp_create_nonce( 'currency_nonce' ),
	...
	public function init_js_properties() {
		...
		<?php if ( ! empty( $_GET ) ) : ?>
			...
			?>
			woocs_array_of_get = '<?php echo str_replace( '\\', '\\\\', str_replace( "'", '', json_encode( $sanitized_get_array ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>';
		<?php endif; ?>
	...
	public function print_plugin_options() {
		...
		if ( isset( $_POST['woocs_name'] ) and ! empty( $_POST['woocs_name'] ) ) {
			...
			if ( ! current_user_can( 'manage_woocommerce' ) ) {
				wp_die( esc_html__( 'You do not have permission.', 'woocommerce-currency-switcher' ) );
			}
			...
			wp_safe_redirect( admin_url( 'admin.php?page=wc-settings&tab=woocs' ) );
			exit;
		}
		// +++
		...
		// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NotInFooter
		wp_enqueue_script( 'woocs-sd-popup23', WOOCS_LINK . 'js/popup23.js', array(), WOOCS_VERSION );
		wp_enqueue_style( 'woocs-popup-23', WOOCS_LINK . 'css/popup23.css', array(), WOOCS_VERSION );

		// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NotInFooter
		wp_enqueue_script( 'woocs-world-currencies', WOOCS_LINK . 'js/world-currencies.js', array(), WOOCS_VERSION );
		// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NotInFooter
		...
		wp_localize_script(
			'woocommerce-currency-switcher-options',
			'woocs_lang',
			array(
	...
	public function woocommerce_currency_symbol( $currency_symbol ) {
		/*
			// If a specific currency is explicitly requested, respect it
	...
	public function woocommerce_variation_prices( $prices_array ) {
		...
		if ( in_array( $current_currency, $this->no_cents )/* OR $currencies[$this->current_currency]['hide_cents'] == 1 */ ) {
			$precision = 0;
		} elseif ( $current_currency != $this->default_currency ) {
	...
	public function woocommerce_variation_prices_fix_3_3( $price, $product_id, $type ) {
		// fix 3.3.3
	...
	public function raw_woocommerce_price( $price, $product = null, $min_max = null, $display = null ) {
		...
		if ( in_array( $this->current_currency, $this->no_cents )/* OR $currencies[$this->current_currency]['hide_cents'] == 1 */ ) {
			$precision = 0;
		} elseif ( $this->current_currency != $this->default_currency ) {
	...
	// +++++++++++++++++++++++++++++ FIINISH: USES ONLY FOR WOO > 2.7 AS FIX ON THE CHECKOUT FOR VARIABLE PRODUCTS ++++++++++++++++++++++++++++++++
	// for tooltip
	private function _get_min_max_variation_prices( $product, $current_currency ) {
		$currencies       = $this->get_currencies();
		$prices_array_old = $product->get_variation_prices();
		$prices_array     = array();
		$var_products_ids  = array_keys( $prices_array_old['regular_price'] ?? array() );
			...
			foreach ( $var_products_ids as $var_prod_id ) {
				...
				if ( $this->is_fixed_enabled ) {
					...
					if ( (int) $fixed_sale_price !== -1 ) {
						...
						$is_price_custom = true;
					} elseif ( (int) $fixed_regular_price !== -1 ) {
		...
		if ( ! empty( $prices_array ) ) {
			...
			if ( ! empty( $prices_array ) ) {
				return array(
					'min' => min( $prices_array ),
					'max' => max( $prices_array ),
				);
			}
		}
	...
	public function woocs_shortcode( $args ) {
		...
		if ( isset( $args['sd'] ) and intval( $args['sd'] ) > 0 ) {
			wp_enqueue_style( 'woocs-sd-selectron23', WOOCS_LINK . 'css/sd/selectron23.css', array(), WOOCS_VERSION );
			// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NotInFooter
			wp_enqueue_script( 'woocs-sd-selectron23', WOOCS_LINK . 'js/sd/selectron23.js', array(), WOOCS_VERSION );
			// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NotInFooter
			wp_enqueue_script( 'woocs-sd-front', WOOCS_LINK . 'js/sd/front.js', array( 'woocs-sd-selectron23' ), WOOCS_VERSION );

			if ( $this->shop_is_cached ) {
				// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NotInFooter
				wp_enqueue_script( 'woocs-sd-front-cache', WOOCS_LINK . 'js/sd/front-cache.js', array( 'woocs-sd-front' ), WOOCS_VERSION );
			}
	...
	public function get_rate() {
		...
		if ( $is_ajax ) {
			check_ajax_referer( 'currency_nonce', 'currency_nonce' );
			if ( ! current_user_can( 'manage_woocommerce' ) ) {
	...
	public function save_etalon() {
		...
		if ( ! ( isset( $_POST['currency_nonce'] ) && wp_verify_nonce( wp_unslash( $_POST['currency_nonce'] ), 'currency_nonce' ) ) ) {
			die();
		}
		// $this->make_rates_auto_update(true);
		$currencies    = $this->get_currencies();
	...
	public function download_flags( $key ) {
		...
		$image_url = 'https://flagcdn.com/w80/' . strtolower( $key ) . '.png';
		// Fallback link if download fails
		$image_link = 'https://flagcdn.com/w80/' . strtolower( $key ) . '.png';
	...
	function apply_conversion( $paypal_args ) {
		if ( in_array( $this->current_currency, $this->no_cents ) ) {
			...
			foreach ( $paypal_args as $key => $value ) {
				...
				} elseif ( strpos( $key, 'tax_cart' ) !== false ) {
						$paypal_args[ $key ] = number_format( $value, 0, $this->decimal_sep, '' );
				}
			}
		}
			...
			foreach ( $currencies as $сurr ) {
				...
				if ( $product_type == 'variable' ) {
					...
					foreach ( $child_ids as $prod_id ) {
						...
						} elseif ( $this->is_fixed_enabled and $this->is_multiple_allowed ) {
					...
					$info .= '<li><span>' . $сurr['name'] . '</span>: ' . $this->wc_price(
						$value,
						false,
						array(
							'currency'           => $сurr['name'],
	...
	public function wc_price( $price, $convert = true, $args = array(), $product = null, $decimals = -1 ) {
		...
		if ( $this->is_geoip_manipulation and ! $is_price_custom and ! is_null( $product ) ) {
			...
			} elseif ( $product_geo_price_data[1] ) {
	...
	public function woocommerce_product_is_on_sale( $value, $product ) {
		...
		} elseif ( $sale_price !== $regular_price and ( $price === $sale_price ) ) {
				$is_sale = true;
		}
	...
	public function recalculate_order( $order_id, $selected_currency = '' ) {
		...
		$line_items = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT order_item_id, order_item_type FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d",
				$order_id
			),
			ARRAY_N
	...
	public function woocommerce_cart_item_price( $product_price, $cart_item, $cart_item_key ) {
		...
		if ( $user_currency != $this->current_currency and ! empty( $user_currency ) ) {
			...
			$wc_price       = $this->wc_price(
				$cart_price,
				true,
				array(
					'thousand_separator' => $thousand_separator,
					'decimal_separator'  => $decimal_separator,
					'decimals'           => $this->get_currency_price_num_decimals( $user_currency, $this->price_num_decimals ),
				)
			);
	...
	public function woocs_fix_decimals( $code ) {
		global $wp_filter;
		$functions = debug_backtrace(); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_debug_backtrace
		foreach ( $functions as $funcs ) {
			if ( $funcs['function'] == 'add_rate' ) {
				return $this->get_currency_price_num_decimals( $this->default_currency );
			}
		}
	...
	public function delete_profiles_data() {
		if ( ! ( isset( $_POST['woocs_wpnonce_geo'] ) && wp_verify_nonce( wp_unslash( $_POST['woocs_wpnonce_geo'] ), 'woocs_wpnonce_geo' ) ) ) {
	...
	private function get_rate_provider( $mode ) {
		// TODO  for php < 8.0 :null|\WOOCS\Rates\Aggregators\RateProvider
		$rate_provider = null;
	...
	public function override_my_account_orders( $order ) {
		...
		// translators: %1$s is the formatted order total, %2$s is the number of items.
		echo wp_kses_post( sprintf( _n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'woocommerce-currency-switcher' ), $order->get_formatted_order_total(), $item_count ) );
	}
	...
	public function rest_get_product_price( $request ) {
		...
		if ( ! $product ) {
			return new WP_Error( 'product_not_found', esc_html__( 'Product not found', 'woocommerce-currency-switcher' ), array( 'status' => 404 ) );
		}
		...
		foreach ( $product_ids as $product_id ) {
			...
			if ( $product ) {
				...
				$results[ $product_id ] = array(
					'error'   => true,
					'message' => esc_html__( 'Product not found', 'woocommerce-currency-switcher' ),
				);
			}
		}
